if (!process.env.SPOTIFY_COOKIE_BASE64)
  throw new Error("Missing SPOTIFY_COOKIE_BASE64 environment variable");
const spotifyCookie = Buffer.from(
  process.env.SPOTIFY_COOKIE_BASE64,
  "base64"
).toString();
...
export async function fetchAccessToken(): Promise<AccessTokenResponse> {
  ...
  if (!json.accessToken)
    throw new Error("Did not receive a Spotify access token");
...
export async function fetchClientToken(
  ...
  if (json.response_type !== "RESPONSE_GRANTED_TOKEN_RESPONSE") {
    throw new Error(
      `Response type was ${json.response_type}, expected RESPONSE_GRANTED_TOKEN_RESPONSE`
    );
  }
...
export async function getValidAccessToken(): Promise<string> {
  if (
    !_cachedAccessToken ||
    Date.now() - tokenGracePeriodMs >
      _cachedAccessToken.accessTokenExpirationTimestampMs
  ) {
